...
| docker / security | [code-rag-bind-mount-ignores-gitignore](./code-rag-bind-mount-ignores-gitignore.md) | A repo bind-mount reads the filesystem, not git — `.gitignore` alone doesn't stop a local indexer from embedding a secret. code-rag's first index leaked `.mcp.json` + a service-account key into Qdrant; fixed with `RESPECT_GITIGNORE` (delegates to `git ls-files --others --ignored --exclude-standard`, fails open off a git repo) |
